...
public:
    ...
    {
        cout<<"+----------------------------------------------------+"<<endl;
        cout<<"+    Tic-Tac-Toe with heuristic and Depth limit(3)   +"<<endl;
        cout<<"+----------------------------------------------------+"<<endl<<endl<<endl;
    }
    void display()
    {
        ...
        for(int i=0;i<3;i++)
            for(int j=0;j<3;j++)
            {
                if(i==1)
                {
                   ...
                   if(sq[i][j]=='X')
                   {
                       if(sq[i-1][j]=='X' && sq[i+1][j]=='X')
                        eval+=100;
                       else if((sq[i-1][j]=='X' && sq[i+1][j]=='.') || (sq[i-1][j]=='.' && sq[i+1][j]=='X'))
                        eval+=50;
                       else if((sq[i-1][j]=='O' && sq[i+1][j]=='O') )
                        eval+=10;
                        if(i==j)
                        {
                            eval+=10;
                            if(sq[i-1][j-1]=='X' && sq[i+1][j+1]=='X')
                                eval+=100;

                            else if((sq[i-1][j-1]=='X' && sq[i+1][j+1]=='.') || (sq[i-1][j-1]=='.' && sq[i+1][j+1]=='X'))
                                eval+=50;
                            else if(sq[i-1][j-1]=='O' && sq[i+1][j+1]=='O')
                                eval+=10;
                            if(sq[i+1][j-1]=='X' && sq[i-1][j+1]=='X')
                                eval+=100;
                   ...
                   {
                       if(sq[i-1][j]=='O' && sq[i+1][j]=='O')
                        eval-=100;
                       else if((sq[i-1][j]=='O' && sq[i+1][j]=='.') || (sq[i-1][j]=='.' && sq[i+1][j]=='O'))
                        eval-=50;
                       else if(sq[i-1][j]=='X' && sq[i+1][j]=='X' )
                        eval-=10;
                        if(i==j)
                        {
                            eval-=10;
                            if(sq[i-1][j-1]=='O' && sq[i+1][j+1]=='O')
                                eval-=100;

                            else if((sq[i-1][j-1]=='O' && sq[i+1][j+1]=='.') || (sq[i-1][j-1]=='.' && sq[i+1][j+1]=='O'))
                                eval-=50;
                            else if(sq[i-1][j-1]=='X' && sq[i+1][j+1]=='X')
                                eval-=10;
                            if(sq[i+1][j-1]=='O' && sq[i-1][j+1]=='O')
                                eval-=100;
               ...
               if(j==1)
                   {
                        if(sq[i][j]=='X')
                       {
                           if(sq[i][j-1]=='X' && sq[i][j+1]=='X')
                            eval+=100;
                           else if((sq[i][j-1]=='X' && sq[i][j+1]=='.') || (sq[i][j-1]=='.' && sq[i][j+1]=='X'))
                            eval+=50;
                           else if((sq[i-1][j]=='O' && sq[i+1][j]=='O') )
                            eval+=10;
...
int minimax(int v, bool isMax, Square Tsq,int mov)
{
    ...
    if(mov==3) return Tsq.evalFunc();
    else if(mov<3 && Tsq.isTerminalState(Tsq)) return Tsq.evalFunc();
    if(isMax)
    {
        for(int i = 0 ;i < 9 ; i++)
        {
...
int main()
{
    ...
    while(!SQ.isTerminalState(SQ))
    {
        ...
        else
        {
            ...
            {
                for(int i=0;i<9;i++) utility[i]=0; //utility of each square is set to -10, each is set to some value after minimax is called

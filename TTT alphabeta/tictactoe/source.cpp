...
int alphabeta(int v, bool isMax, Square Tsq,int mov,int alpha,int beta)
{
    ...
    if(isMax)
    {
        for(int i = 0 ;i < 9 ; i++)
        {
            ...
            if(temp.sq[i/3][i%3]=='.')
            {
                ...
                //Tsq.display();
                int u = alphabeta(infinity,!isMax,temp,mov+1,alpha,beta);
                v= max(v , u);
                if(mov==1)
                    utility[i] = u;
                if(v>=beta) return v;
        ...
        for(int i = 0 ;i < 9 ; i++)
        {
            ...
            if(temp.sq[i/3][i%3]=='.')
            {
                temp.human_fill(i);
                //Tsq.display();
                int u = alphabeta(-1*infinity,!isMax,temp,mov+1,alpha, beta);
                v =  min(v , u);
                if(v<=alpha) return v;
                beta=min(beta , v);
            }
    ...
    while(!SQ.isTerminalState(SQ))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                int x = alphabeta(-1*infinity,true,SQ,1,-1*infinity,infinity); // x stores the highest utility value for all the child nodes from current node
